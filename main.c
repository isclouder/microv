...
static int vcpu_exec(struct VCPUState *vcpu)
{
    ...
    do{
	    ...
	    DPRINTF("hlt\n");
            return 0;
        case KVM_EXIT_IO:
            if(MMIO_SERIAL_ADDR<=run->io.port && \
               run->io.port<MMIO_SERIAL_ADDR+MMIO_SERIAL_SIZE){
...
static void run_simple_bios() { 
    load_kernel(VMLINUX_FILE);
    ...
    _test_boot_params();
    /*
    for(int i=0xfec00000; i<0xfee00000; i++){
         uint8_t str = *(uint8_t *)get_userspace_addr(i);
         printf("Ok(%lu)\n",str);
    }
    */
